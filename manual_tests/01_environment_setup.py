...
def test_demo_data_availability():
    ...
    demo_repos = [
        project_root / "data/pipelines_check-29.1.1/DATA_REPO"
    ]
    ...
    for repo_path in demo_repos:
        ...
        if repo.exists():
            ...
            if butler_yaml.exists():
                print(f"   └─ butler.yaml: OK")
            else:
                print(f"   └─ butler.yaml: MISSING")
...
def main():
    ...
    try:
        ...
        print(f"Environment setup: {'OK' if env_ok else 'FAIL'}")
        print(f"LSST imports: {'OK' if lsst_success else 'FAIL'}")
        print(f"RIPPLe imports: {'OK' if ripple_success else 'FAIL'}")
        print(f"Demo data: {'OK' if demo_available else 'FAIL'}")
