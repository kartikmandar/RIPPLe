...
class DataIngestor:
    ...
    def ingest_calibrations(self) -> Dict[str, Any]:
        ...
        for calib_type, patterns in calib_types.items():
            ...
            try:
                logger.debug(f"Running subprocess for {calib_type} ingestion")
                result = subprocess.run(
                    cmd,
                    capture_output=True,
                    text=True,
                ...
                logger.debug(f"Running subprocess for {calib_type} certification")
                ...
                logger.error(error_msg)
                results["errors"].append(error_msg)
                # Don't increment total_ingested since ingestion failed
        
        logger.debug(f"Total ingested calibrations: {total_ingested}")
        logger.debug(f"Calibration errors: {results['errors']}")
    ...
    def _detect_reference_catalogs(self, refcat_path: Path) -> Dict[str, List[Path]]:
        ...
        for catalog_type, patterns in catalog_patterns.items():
            files = []
            for pattern in patterns:
                found_files = refcat_path.glob(pattern)
